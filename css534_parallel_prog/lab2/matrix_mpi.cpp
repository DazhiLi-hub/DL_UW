...
int main( int argc, char* argv[] ) {
  ...
  if (my_rank == 0) {
    for (int i=1; i<mpi_size;i++) {
      double *partition_a = a + stripe*size*i;
      MPI_Send(partition_a, stripe*size, MPI_DOUBLE, i, 0, MPI_COMM_WORLD);
    }
  } else {
  ...
  if (my_rank !=0) {
    ...
    for (int i=1; i<mpi_size; i++) {
      MPI_Recv(c+stripe*size*i, stripe*size, MPI_DOUBLE, i, 0, MPI_COMM_WORLD, &status);
    }
  }
